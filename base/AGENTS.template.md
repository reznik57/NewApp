...
<!--
  AGENTS.md TEMPLATE — fill per SETUP.md step 7 (the fill checklist and
  the post-fill line budget live there; this comment block is deleted).
  This file is the CANONICAL instruction home every agent tool reads;
  CLAUDE.md stays a thin bridge that imports it. Load-bearing principles:
  Invariants 3 and 4 (Source of Truth, Symbols-Not-Counts). Keep them even
...
- Run `{{CHECK CMD — e.g. npm run check}}` (format + lint + typecheck + fast
  ...
  **0 errors, 0 warnings**. Under Claude Code the Stop hook
  ...
  with CI as the backstop; where no hook fires (e.g. Cowork/claude.ai, or an
  agent tool that reads this file but registers no hooks) both halves are
  instruction. Don't argue with a red gate — fix the failure.
...
- **Design before UI**: building or reshaping a user-facing surface? Invoke
  the `frontend-design` skill (canonical copy in `.agents/skills/`) BEFORE
  writing markup or styles — it steers palette, type and layout away from
  templated defaults. A constraint profile's skill layers ON TOP of it, never
  instead.
  <!-- ADAPT (no UI at all — a library, a service, a CLI): delete this bullet
  AND both `.agents/skills/frontend-design/` and its Claude bridge at
  `.claude/skills/frontend-design/`. A skill nothing invokes is dead weight
  that misleads by existing. -->
