---
name: log-gotcha
description: Capture a hard-won lesson at the end of an incident, bug hunt, or debugging session — or record a deliberate shortcut (deferred debt) that shipped. Files it in the right knowledge home (AGENTS.md gotcha one-liner, wiki page, deferred-debt log entry, or a proposed hook/lint rule) and updates the wiki index and log.
---
...
1. **Write the lesson** in Problem→Rule form (format spec: the Project
   Gotchas comment in AGENTS.md).
2. **Choose the home** (graduation rule):
   - First occurrence, fits in ≤3 lines → one-liner in AGENTS.md → Project
     Gotchas.
   ...
   - Recurring (second+ occurrence — check `docs/wiki/log.md`) → make sure
     the AGENTS.md one-liner exists and is sharp.
...
3. **Update the registers**: add any new page to `docs/wiki/index.md`; append
   a `learned` entry to `docs/wiki/log.md` (format: AGENTS.md → Docs &
   Knowledge Schema).
