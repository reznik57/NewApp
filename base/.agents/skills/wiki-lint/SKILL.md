---
name: wiki-lint
description: Docs health check for AGENTS.md, ADRs, and the wiki. Finds dead symbol references, orphan or unindexed pages, contradicted decisions without superseded-by links, duplicated facts, leftover placeholders, and an AGENTS.md over its line budget. Run monthly or before major work.
---
...
1. **Dead references**: extract every code symbol cited in `AGENTS.md`,
...
4. **Duplication drift**: any fact stated in both AGENTS.md and a wiki/ADR
...
5. **Leftovers**: run `.claude/scripts/check_markers.py` — it must print
   ...
   their single home).
6. **AGENTS.md budget**: over ~190 lines (single home of this number —
