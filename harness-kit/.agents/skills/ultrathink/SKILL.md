...
**When to run**: the trigger list lives in AGENTS.md → Deep-Analysis Protocol
...
Open AGENTS.md and read the **⛔ Critical Invariants now** — every one of
...
| Does it pull in a NEW dependency?                    | Apply AGENTS.md → Standing Rules                  |
...
- New external dependency? Apply AGENTS.md → Standing Rules.
- Existing pattern to reuse? Search before creating.
- Derive integration points fresh from AGENTS.md → Architecture — do NOT
...
- Deviations are data: the moment execution forces a change to an approved
  ...
  contradicts the accepted ADR follows AGENTS.md → ADR discipline.
...
Governed by AGENTS.md → ⛔ Critical Invariants (Verification Gate + Test
Integrity).

---

If this protocol missed something, update this file — cite symbols, never
counts or line numbers (AGENTS.md Invariant 4).
